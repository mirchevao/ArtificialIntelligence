...
def move_obstacle1(positioj):
    x, y, direction = position
    if (0 < x < 6 and direction == 1) or ( 0 < x < 6 and direction == -1):
        direction = direction * (-1)
    x_new = x + direction
    position_new = x_new, y, direction
    return position_new

def move_obstacle2(position):
    x,y,direction = position
    if(1 < x < 7 and 1 < y < 7 and direction == 1) or (1 < x < 7 and 1 < y < 7 and direction == -1):
        direction = direction * (-1)
    x_new = x + direction
    y_new = y + direction
...
def move_obstacle3(position):
    x, y, direction = position
    if (0 < y < 6 and direction == 1 ) or ( 0 < y < 6 and direction == -1):
        direction = direction*(-1)
    y_new = y + direction
...
class Expl(Problem):
    ...
    def successor(self, state):
        ...
        obstacle3 = (state[8], state[9], state[10])

        #right
        if y-1 >=0:
            x_new = x + 1
            y_new = y
            obstacle1_new = move_obstacle1(obstacle1)
            obstacle2_new = move_obstacle2(obstacle2)
            obstacle3_new = move_obstacle3(obstacle3)

            if(x_new != obstacle1_new[0] or y_new != obstacle1_new[1]) and (x_new != obstacle2_new[0] or y_new != obstacle2_new[1]) and (x_new != obstacle3_new[0] or y_new != obstacle3_new[1]):
                state_new = (x_new, y_new, obstacle1_new[0], obstacle1_new[1], obstacle1_new[2], obstacle2_new[0], obstacle2_new[1], obstacle2_new[2], obstacle3_new[0], obstacle3_new[1], obstacle3_new[2])
                suc['Right'] = state_new
        #left
        if x < 5 and y < 5:
            x_new = x-1
        ...
        if x < 10:
            ...
            if (x_new != obstacle1_new[0] or y_new != obstacle1_new[1]) and (
                    x_new != obstacle2_new[0] or y_new != obstacle2_new[1]) and (
                    x_new != obstacle3_new[0] or y_new != obstacle3_new[1]):
                state_new = (
                x_new, y_new, obstacle1_new[0], obstacle1_new[1], obstacle1_new[2], obstacle2_new[0], obstacle2_new[1],
                obstacle2_new[2], obstacle3_new[0], obstacle3_new[1], obstacle3_new[2])
                suc['Up'] = state_new
...
IstrazuvacInstance = Expl((CoveceRedica, CoveceKolona, (2, 2, -1), (7, 2, 1), (7, 8, 1)),
