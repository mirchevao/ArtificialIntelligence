...
class GraphProblem(Problem):
    ...
    def h(self, node):
        ...
        if locs:
            ...
            return math.inf
#moving knight type 1 up+up+left
def move_knight_type1(position):
    x = position[0]
    y = position[1]
    if( x > 1 and y > 2) and ((x-1, y+2) != (position[2], position[3])):
        x = x-1
        y = y + 2
    position_new = x,y
    return position_new
def move_knight_type2(position):
    x = position[0]
    y = position[1]
    if(x > 1 and y > 2) and ((x+1, y+2) != (position[2], position[3])):
        x = x+1
    ...
    position_new = x,y
    return position_new
def move_knight_type3(position):
    x = position[0]
    y = position[1]
    if (x > 2 and y > 1) and ((x+2, y+1) != (position[2], position[3])):
        x = x+2
        y = y+1
    position_new = x,y
    return position_new
def move_knight_type4(position):
    x = position[0]
    y = position[1]
    if(x>2 and y>1) and ((x+2, y-1) != (position[2], position[3])):
        x = x+2
    ...
    position_new = x,y
    return position_new
def move_knight_type5(position):
    x = position[0]
    y = position[1]
    if(x > 1 and y > 2) and ((x+1, y-2) != (position[2], position[3])):
        x = x+1
    ...
    position_new=x,y
    return position_new
def move_knight_type6(position):
    x = position[0]
    y = position[1]
    if(x>1 and y>2) and ((x-1, y-2) != (position[2], position[3])):
        x = x-1
    ...
    position_new = x,y
    return position_new
def move_knight_type7(position):
    x = position[0]
    y =position[1]
    if(x > 2 and y > 1) and ((x-2, y-1) != (position[2], position[3])):
        x = x-2
    ...
    position_new = x,y
    return position_new
def move_knight_type8(position):
    x = position[0]
    y = position[1]
    if(x>2 and y>1) and ((x-2, y +1), (position[2], position[3])):
        x = x-2
    ...
    position_new = x,y
    return position_new
def move_bishop_type1(position):
    x = position[2]
    y = position[3]
    if( 0 < x < 9 and 0 < y < 9) and ((x-1, y+1) != (position[0], position[1])):
        x = x-1
        y = y+1
    position_new = x,y
    return position_new
def move_bishop_type2(position):
    x = position[2]
    y = position[3]
    if( 0 < x < 9 and 0 < y < 9 ) and ((x+1, y+1) != (position[0], position[1])):
        x = x+1
    ...
    position_new = x,y
    return position_new
def move_bishop_type3(position):
    x= position[2]
    y=position[3]
    if( 0 < x < 9 and 0 < y < 9 ) and ((x-1, y-1) != (position[0], position[1])):
        x = x-1
    ...
    position_new = x,y
    return position_new
def move_bishop_type4(position):
    x = position[2]
    y = position[3]
    if( 0 < x < 9 and 0 < y < 9 ) and ((x+1, y-1) != (position[0], position[1])):
        x = x+1
...
class Star(Problem):
    ...
    def successor(self, state):
        suc = dict()
        knight = state[0], state[1]
        bishop = state[2], state[3]
        star = state[4]

        #knight type1
        knight_new = move_knight_type1(state)
        stars_new = tuple(s for s in star if s != knight_new)
        state_new = knight_new[0], knight_new[1], bishop[0], bishop[1], stars_new
        if state_new != state:
            suc['K1'] = state_new

       #knight type2
        knight_new = move_knight_type2(state)
        stars_new = tuple(s for s in star if s != knight_new)
        state_new = knight_new[0], knight_new[1], bishop[0], bishop[1], stars_new
        if state_new != state:
            suc['K2'] = state_new
        knight_new = move_knight_type3(state)
        stars_new = tuple(s for s in star if s != knight_new)
        state_new = knight_new[0], knight_new[1], bishop[0], bishop[1], stars_new
        if state_new != state:
            suc['K3'] = state_new
        knight_new = move_knight_type4(state)
        stars_new = tuple(s for s in star if s != knight_new)
        state_new = knight_new[0], knight_new[1], bishop[0], bishop[1], stars_new
        if state_new != state:
            suc['K4'] = state_new
        knight_new = move_knight_type5(state)
        stars_new = tuple(s for s in star if s != knight_new)
        state_new = knight_new[0], knight_new[1], bishop[0], bishop[1], stars_new
        if state_new != state:
            suc['K5'] = state_new
        knight_new = move_knight_type6(state)
        stars_new = tuple(s for s in star if s != knight_new)
        state_new = knight_new[0], knight_new[1], bishop[0], bishop[1], stars_new
        if state_new != state:
            suc['K6'] = state_new
        knight_new = move_knight_type7(state)
        stars_new = tuple(s for s in star if s != knight_new)
        state_new = knight_new[0], knight_new[1], bishop[0], bishop[1], stars_new
        if state_new != state:
            suc['K7'] = state_new
        knight_new = move_knight_type8(state)
        stars_new = tuple(s for s in star if s != knight_new)
        state_new = knight_new[0], knight_new[1], bishop[0], bishop[1], stars_new
        if state_new != state:
            suc['K8'] = state_new
        ...
        #bishop type 1
        bishop_new = move_bishop_type1(state)
        stars_new = tuple(s for s in star if s != bishop_new)
        state_new = knight[0], knight[1], bishop_new[0], bishop_new[1], stars_new
        if state_new != state:
            suc['B1'] = state_new
        bishop_new = move_bishop_type2(state)
        stars_new = tuple(s for s in star if s != bishop_new)
        state_new = knight[0], knight[1], bishop_new[0], bishop_new[1], stars_new
        if state_new != state:
            suc['B2'] = state_new
        bishop_new = move_bishop_type3(state)
        stars_new = tuple(s for s in star if s != bishop_new)
        state_new = knight[0], knight[1], bishop_new[0], bishop_new[1], stars_new
        if state_new != state:
            suc['B3'] = state_new
        bishop_new = move_bishop_type4(state)
        stars_new = tuple(s for s in star if s != bishop_new)
        state_new = knight[0], knight[1], bishop_new[0], bishop_new[1], stars_new
        if state_new != state:
            suc['B4'] = state_new
...
knight_column = int(input())
bishop_row = int(input())
bishop_column = int(input())
star1_row = int(input())
star1_column = int(input())
star2_row = int(input())
star2_column = int(input())
star3_row = int(input())
star3_column = int(input())
stars_positions = ((star1_row, star1_column), (star2_row, star2_column),
(star3_row, star3_column))
star = Star((knight_row, knight_column, bishop_row,bishop_column, stars_positions))
answer = breadth_first_graph_search(star)
