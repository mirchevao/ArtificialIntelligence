...
def move_obstacles(position):
    x, y, direction = position
    #direction=1 za nadolu
    if(y == 6 and direction == -1) or (y == 0 and direction == 1):
...
class Explorer(Problem):
    ...
    def successor(self, state):
        ...
        if x <= 8 and x >=0:
            x_new = x + 1
            y_new = y
            obstacle1_new = move_obstacles(obstacle1)
            obstacle2_new = move_obstacles(obstacle2)

            if(x_new != obstacle1_new[0] or y_new != obstacle1_new[1]) and (x_new!=obstacle2_new[0] or y_new != obstacle2_new[1]):
                state_new= (x_new, y_new, obstacle1_new[0], obstacle1_new[1], obstacle1_new[2], obstacle2_new[0], obstacle2_new[1], obstacle2_new[2])
                suc['Right'] = state_new
        ...
        if x <= 8 and y >= 0:
            ...
            if (x_new != obstacle1_new[0] or y_new != obstacle1_new[1]) and (
                    x_new != obstacle2_new[0] or y_new != obstacle2_new[1]):
                state_new = (
        ...
        if y <= 6 and y >=0:
            ...
            if (x_new != obstacle1_new[0] or y_new != obstacle1_new[1]) and (
                    x_new != obstacle2_new[0] or y_new != obstacle2_new[1]):
                state_new = (
                    x_new, y_new, obstacle1_new[0], obstacle1_new[1], obstacle1_new[2], obstacle2_new[0],
                    obstacle2_new[1],
                    obstacle2_new[2])
                suc['Up'] = state_new
...
explorer = Explorer((man_row,man_col, 2, 6, 1, 6, 0, -1), house)
