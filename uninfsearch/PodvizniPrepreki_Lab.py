...
def move_obstacle2(obstacle2):
    x = obstacle2[0]
    y = obstacle2[1]
    n = obstacle2[2]

    if(x == 5 and y ==4 and n == -1) or (x == 9 and y == 0 and n ==1):
        n = n*(-1)
    x_new = x+n
    y_new = y-n
    return (x_new, y_new, n)

def move_obstacle3(obstacle3):
    x = obstacle3[0]
    n = obstacle3[2]

    if ( x == 9 and n == 1) or (x == 5 and n == -1):
        n = n*(-1)
    x_new = x + n
    return x_new
