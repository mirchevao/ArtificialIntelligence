...
for i in range(len(networks)):
    ...
    for row in validation_set:
        preditcion = predict(networks[i], row)
        if(preditcion == row[-1]): #ako predikcijata e tocna za klasata oti klasata e u posleden red
            count+=1
        if count > minN:
            minN=count
            index=i
finalprediction =  predict(networks[index], testcase)
