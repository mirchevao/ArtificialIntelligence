...
def move_atom_up(atom1, atom2, atom3):
    x = atom1[0]
    y = atom1[1]
    while(0 < x < 10 and 0 < y < 8) and (atom1 not in obstacles) and (atom1 not in [atom2, atom3]):
        position_new = x, y+1
    return position_new
...
class Molecule(Problem):
    ...
    def successor(self, state):
        ...
        state_new = h1 + o + h2
        if state_new != state:
            suc['H2Down'] = state_new
        #move h2 left
        h2_new = move_atom_left(h2, o, h1)
        state_new = h1 + o + h2
        if state_new != state:
            suc['H2Left'] = state_new
        #move h2 right
        h2_new = move_atom_right(h2, o, h1)
        state_new = h1 + o + h2
        if state_new != state:
            suc['H2Right'] = state_new
