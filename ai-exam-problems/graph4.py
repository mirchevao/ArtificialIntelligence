...
class GraphProblem(Problem):
    ...
    def successor(self, state):
        ...
        recnik = dict()
        pom = self.graph.get(state[0])

        for el in pom:
            listState = [state[0], list(state[1])]
            if el in listState[1]:
                listState[1].remove(el)
            listState[0] = el
            recnik[el] = (el, tuple(listState[1]))
        return recnik
    ...
    def h(self, node):
        ...
        if locs:
            min = -10000
            for el in node.state[1]:
                if min < int(distance(locs[node.state[0]], locs[el])):
                    min = int(distance(locs[node.state[0]], locs[el]))
            return min
        else:
            return math.inf

grap_map = UndirectedGraph(dict(
        A=dict(B=8, C=4, E=10),
        C=dict(D=6),
        B=dict(E=6),
        E=dict(F=4,J=4,H=8),
...
locations = dict(
    A=(6, 0), B=(2, 6), C=(8, 3),
    D=(13, 5), E=(7, 9), F=(10, 8),
    G=(16, 4), H=(11, 15), I=(17, 13),
    J=(4, 11), K=(7, 19), L=(5, 20),
    M=(9, 23), N=(15, 25), O=(3, 18),
    P=(6, 30)
)
grap_map.locations=locations
pocetok = input()
torka = input().split(',')
torka = tuple(torka)

problem = GraphProblem((pocetok,torka),None, grap_map)
answer=astar_search(problem)
lista=[]
for el in answer.solve():
    lista.append(el[0])
print("Dolzhina: " + str(answer.path_cost) + ", Pateka: " + str(lista))