...
#klasa e vo posledna kolona
#da se zemat privte 5 od sekoja klasa
test_case = input()
list1 = [x for x in data if x[-1] == 'Roach']
list2 = [x for x in data if x[-1] == 'Perch']
list3 = [x for x in data if x[-1] == 'Bream']
list4 = [x for x in data if x[-1] == 'Whitewish']
list5 = [x for x in data if x[-1] == 'Silver Bream']
list6 = [x for x in data if x[-1] == 'Pike']
list7 = [x for x in data if x[-1] == 'Smelt']

training_data = list1[:5] + list2[:5] +  list3[:5] + list4[:5] + list5[:5] + list6[:5] + list7[:5]
tree = build_tree(data)
for test_case in training_data:
    print("Nepoznat slucaj:" , test_case , "Klasifikacija" ,classify(test_case,tree))
#print_tree(tree)
