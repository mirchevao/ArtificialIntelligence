...
training_num = (1-percent)*30
validation_num = percent*30

training_set = dataset[:int(training_num)]
validation_set = dataset[int(training_num):int(validation_num)]
...
for row in validation_set:
    if predict(network, row) == row[-1]:
        correct+=1
total_correct = correct / len(training_set)

correct2=0
for i in dataset:
    training_set2 = [extract_column(row, i) for row in training_set]
    validation_set2 = [extract_column(row, i) for row in validation_set]
    network2 = initialize_network(n_inputs, n_hidden, n_outputs)
    train_network(network2, training_set2, learning_rate, epoch, n_outputs)
    for j in validation_set2:
        if predict(network2, j) == j[-1]:
            correct2+=1
total_correct2 = correct2/len(training_set2)
print(total_correct)
