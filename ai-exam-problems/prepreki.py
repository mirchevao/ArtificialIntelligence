...
def moveO2(obstacle):
    ...
    y = obstacle[1]
    direction = obstacle[2]
    if (x == 1 and y == 2 and direction == 1) or (x == 5 and y == 6 and direction == -1):
        direction*=(-1)
    x_new = x - direction
    y_new = y - direction
...
def isValid(x, y, p1, p2, p3):
    if (x == p1[0] and y == p1[1]) or (x == p1[0] + 1 and y == p1[1]):
        return False
    if (x == p3[0] and y == p3[1]) or (x == p3[0] and y == p3[1] + 1):
        return False
...
class PodvizniPrepreki(Problem):
    ...
    def successor(self, state):
        ...
        if ( x < 6 ) or ( x < 12):
            x_new = x - 1
            obstacle1_new = moveO1(obstacle1)
            obstacle2_new = moveO2(obstacle2)
            obstacle3_new = moveO3(obstacle3)
            if (isValid(x_new, y, obstacle1_new, obstacle2_new, obstacle3_new)):
                state_new = x_new, y, obstacle1_new, obstacle2_new, obstacle3_new
                suc['Levo'] = state_new
        #desno
        if (x > 6 ) or ( x > 12):
            x_new = x + 1
        ...
        if (y > 0) or (y > 6):
            ...
            obstacle2_new = moveO2(obstacle2)
            obstacle3_new = moveO3(obstacle3)
            if (isValid(x, y_new, obstacle1_new, obstacle2_new, obstacle3_new)):
                state_new = x, y_new, obstacle1_new, obstacle2_new, obstacle3_new
                suc['Gore'] = state_new
        #dolu
        if (y < 12) or (y < 6):
            y_new = y - 1
...
p = PodvizniPrepreki((CoveceRedica, CoveceKolona, (3, 9, 1), (3, 4, 1), (9, 3, 1)), kuka)
answer = breadth_first_graph_search(p)
