...
def gore(a):
    covece = a[0]
    if (covece >= 1):
        if ((covece, covece+3) in rebra or (covece+3, covece) in rebra):
            rebra.remove(covece, covece+3)
            rebra.remove(covece+3, covece)
            covece_new = covece+3
            return covece_new
    return covece
...
def dolu(a):
    covece = a[0]
    if (covece <= 16):
        if((covece, covece-3) in rebra or (covece+4, covece) in rebra):
            rebra.remove(covece,covece-3)
            rebra.remove(covece-3, covece)
            covece_new = covece-3
        return covece_new
...
def levo(a):
    ...
    if (covece not in (1, 5, 9, 13)):
        if ((covece, covece-1) not in rebra or (covece-1, covece) not in rebra):
...
def goreLevo(a):
    covece = a[0]
    if covece == 7:
        if ((covece, covece+3) not in rebra or (covece+3, covece) not in rebra):
            rebra.remove(covece, covece+3)
            rebra.remove((coveve+3, covece))
            covece_new = covece+3
        return covece_new
    return covece
...
class Zvezdi(Problem):
    ...
    def successor(self, state):
        ...
        #gore
        igrac_new = gore(igrac)
        zvezda_new = [z for z in zvezdi if z!=igrac_new]
        state_new = igrac_new, zvezda_new
        suc['Gore']=state_new
        #dolu
        igrac_new = dolu(igrac)
        zvezda_new = [z for z in zvezdi if z != igrac_new]
        state_new = igrac_new, zvezda_new
        suc['Dolu'] = state_new
        #levo
        igrac_new = levo(igrac)
        zvezda_new = [z for z in zvezdi if z != igrac_new]
        state_new = igrac_new, zvezda_new
        suc['Levo'] = state_new
        #desno
        igrac_new = desno(igrac)
        zvezda_new = [z for z in zvezdi if z != igrac_new]
        state_new = igrac_new, zvezda_new
        suc['Desno'] = state_new
        #doludesno
        igrac_new = doluDesno(igrac)
        zvezda_new = [z for z in zvezdi if z != igrac_new]
        state_new = igrac_new, zvezda_new
        suc['doluDesno'] = state_new
        #gore levo
        igrac_new = goreLevo(igrac)
        zvezda_new = [z for z in zvezdi if z != igrac_new]
        state_new = igrac_new, zvezda_new
        suc['GoreLevo'] = state_new
