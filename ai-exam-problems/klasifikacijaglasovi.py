...
testcase = [attr1, attr2, attr3, attr4, attr5]

l = len(dataset)
set1 = l*0.3
set2 = set1 * 2
training_set1 = dataset[:int(set1)]
training_set2 = dataset[int(set1):int(set2)]
training_set3 = dataset[int(set2):]

tree1 = build_tree(training_set1)
tree2 = build_tree(training_set2)
tree3 = build_tree(training_set3)

vote1 = 0
vote2 = 0
vote3 = 0

prediction1 = classify(testcase, tree1)
prediction2 = classify(testcase, tree2)
prediction3 = classify(testcase, tree3)

key1 = sorted([*prediction1.keys()], key = prediction1.get, reverse = True)
key2 = sorted([*prediction2.keys()], key = prediction2.get, reverse = True)
key3 = sorted([*prediction3.keys()], key = prediction3.get, reverse = True)

if key1[0] >= key2[0] and key1[0] >= key3[0]:
    print(key1[0])
elif key2[0]>= key1[0] and key2[0] >= key3[0]:
    print(key2[0])
elif key3[0]  >= key1[0] and key3 >= key2[0]:
    print(key3[0])
