...
def classify(observation, tree):
    if tree.results!=None:
        ...
        print("Sporeduvam kolona i vrednost (" + str(tree.col) + str(tree.value) )
        print("Tekovna vrednost" + value)
        br=-1
        if isinstance(value, int) or isinstance(value, float):
            if value >= tree.value:
                branch=tree.tb
            ...
            if value == tree.value:
                ...
                br=2
        if br == 1:
            print("Sledna granka e true branch")
        elif br == 2:
            print("sledna granka e false branch")
        print("preostanati za izminuvanje")
...
if __name__ == "__main__":
    ...
    testCase = ['google', 'USA', 'no', 25, serviceChosen]
    # vasiot kod tuka
    # my_data.append(testCase)
    tree = build_tree(my_data)
    # printtree(tree)
    print(classify(test_cases[1], tree))
