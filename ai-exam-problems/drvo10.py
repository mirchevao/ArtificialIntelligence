...
def check_if_votes_are_tie(votes):
    if 2 in votes or 3 in votes:
        return False
    return True
...
if __name__ == '__main__':
    ...
    x4 = float(input())
    x5 = float(input())
    test_case=[x1,x2,x3,x4,x5]

    thirty_percent = len(dataset*0.3)
    forty_percent = len(dataset*0.4)

    dataset1 = dataset[:thirty_percent]
    dataset2 = dataset[thirty_percent:thirty_percent*2]
    dataset3 = dataset[thirty_percent*2:]

    tree1 = build_tree(dataset1)
    tree2 = build_tree(dataset2)
    tree3 = build_tree(dataset3)
    ...
    prediction1 = classify(test_case,tree1)
    prediction2 = classify(test_case,tree2)
    prediction3 = classify(test_case, tree3)
