...
#ako kolonata e razlicna od indeksot vrati ja

column_id = int(input())
num = int(dataset(len)*0.8)
training_set1 = dataset[:num]
validation_set1 = dataset[num:]

training_set2 = [extract_column(row, column_id) for row in training_set1 ]
validation_set2 = [extract_column(row, column_id) for row in validation_set1]
...
for (row1, row2) in zip(validation_set1, validation_set2):
    ...
    if prediction2 == row2[-1]:
        correct2+=1
c1 = correct1/len(training_set1)
c2 = correct2/len(training_set2)
print(c1)
print(c2)
