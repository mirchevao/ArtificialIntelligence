...
data = [[0.02, 0.0371, 0.0428, 0.0207, 0.0954, 0.0986, 0.1539, 0.1601, 0.3109, 0.2111, 0.1609, 0.1582, 0.2238, 0.0645,
         0.066, 0],
...
if __name__ == '__main__':
    ...
    epoch_num = int(input())
    l = len(data)
    training_set = data[:int(l*0.8)]
    validation_set = data[int(l*0.8):]
    n_outputs=2
    network = initialize_network(7, 6, 2)
    train_network(network, training_set, learning_rate, epoch_num,n_outputs)
    count1=0
    for row in training_set:
        predictiont = predict(network, row)
        if predictiont == row[-1]:
            count1+=1
    accuracy1 = count1/len(training_set)

    count2 =0
    for row in validation_set:
        predictionv = predict(network, row)
        if predictionv == row[-1]:
            count2+=1
    accuracy2 = count2/len(validation_set)

    if abs(accuracy1-accuracy2) < 0.15:
        print("Ne se slucuva overfitting")
    else:
        print("Se slucuva overfitting")
    print("Tocnost so training set: ", accuracy1)
    print("Tocnost so validation set: ", accuracy2)