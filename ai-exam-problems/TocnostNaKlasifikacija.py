...
test_data = [train_data[i] for i in range(len(train_data)) if i in test_indices]
training_data=[train_data[i] for i in range(len(train_data)) if i not in test_indices]

half = len(train_data)//2
tree1 = build_tree(train_data[:half])
tree2 = build_tree(train_data[half:])

correct1=0
correct2=0

for row in test_data:
    prediction1 = classify(row, tree1)
    prediction2 = classify(row, tree2)

    if prediction1 == row[-1]:
        correct1+=1
    if prediction2 == row[-1]:
        correct2+=1
c1 = correct1 / len(test_data)
c2 = correct2 / len(test_data)

print(f"Tochnost na drvoto 1: {c1}")
print(f"Tochnost na drvoto 2: {c2}")

if c1 == c2:
    print("Drvata imaat ista tochnost")
elif c1 > c2:
    print("Drvoto 1 ima pogolema tochnost od drvoto 2")
else:
    print("Drvoto 2 ima pogolema tochnost od drvoto 1")
