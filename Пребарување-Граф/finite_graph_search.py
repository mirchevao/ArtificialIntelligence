...
class MojGrafProblem(GraphProblem):

    def __init__(self, initial, goal, graph, Specijalna):
        self.special = Specijalna
        super().__init__((initial, False), goal, graph)

    def actions(self, state):
        """The actions at a graph node are just its neighbors."""
        return list(self.graph.get(state[0]).keys())

    def result(self, state, action):
        """The result of going to a neighbor is just that neighbor."""
        if action == self.special:
            return action, True
        return action, state[1]

    def path_cost(self, c, state1, action, state2):
        if action == self.special and not state1[1]:
            return c + (self.graph.get(state1[0], state2[0]) or math.inf) - 9
        return c + (self.graph.get(state1[0], state2[0]) or math.inf)

    def goal_test(self, state):
        return state[0] == self.goal

    def h(self, node):
        return len(breadth_first_graph_search(GraphProblem(node.state[0], self.goal, graph)).solution())
...
mojProblem = astar_search(MojGrafProblem(Pocetok, Kraj, graph, Specijalna))

print(mojProblem.path_cost)